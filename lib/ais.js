...
export function startAisClient({ key, radiusNm, store }) {
  ...
  // aisstream filtre activement les petits bbox (testé : 50 NM = 0 message en 60s,
  // 6°×9° = 127 messages en 15s). On s'abonne au moins à 200 NM pour que le serveur pousse,
  // et le filtrage à `radiusNm` se fait côté snapshot.
  const SUB_RADIUS_NM = Math.max(radiusNm * 3, 300);
  const RESUB_THRESHOLD_NM = SUB_RADIUS_NM / 5;
  ...
  function subscribe(lat, lon) {
    ...
    const bbox = radiusToBbox(lat, lon, SUB_RADIUS_NM);
    const msg = {
      APIKey: key,
      BoundingBoxes: [bbox],
    ...
    log.info(`Abonnement AIS bbox ${SUB_RADIUS_NM} NM (filtré à ${radiusNm} NM côté snapshot) autour de`, { lat: lat.toFixed(4), lon: lon.toFixed(4) });
  ...
  function handle(raw) {
    ...
    if (type === 'PositionReport') {
      ...
      // Tag source à chaque tour pour que les cibles sans static data aient quand même la source
      store.updateAisStatic(mmsi, { source: 'aisstream' });
      if (meta.ShipName) {
