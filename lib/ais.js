...
export function startAisClient({ key, radiusNm, store }) {
  ...
  function subscribe(lat, lon) {
    ...
    // Format aisstream.io officiel : APIKey + BoundingBoxes (les filtres sont optionnels).
    // Ne PAS envoyer de clés inconnues (genre Apikey k minuscule) — la validation peut échouer silencieusement.
    const msg = {
      APIKey: key,
      BoundingBoxes: [bbox],
