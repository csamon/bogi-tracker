...
export function startAisClient({ key, radiusNm, store }) {
  ...
  // aisstream filtre activement les petits bbox (50 NM = 0 message reçu en pratique).
  // On s'abonne avec un bbox plus large pour que le serveur pousse réellement les messages.
  // Le filtrage à `radiusNm` se fait ensuite côté snapshot dans le store.
  const SUB_RADIUS_NM = Math.max(radiusNm * 4, 200);
  const RESUB_THRESHOLD_NM = SUB_RADIUS_NM / 5; // resub si Mapei a dérivé > 1/5 du rayon abonné
