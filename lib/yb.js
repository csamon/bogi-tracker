...
// Enricher : fetch le détail des points qui n'en ont pas encore, en arrière-plan,
// à un rythme doux pour ne pas surcharger YB. Idéal pour préparer les segments colorés.
export function startTrackEnricher({ keyword, store, intervalMs = 600 }) {
  let stopped = false;

  async function tick() {
    if (stopped) return;
    const id = store.getNextMissingDetailId();
    if (id != null) {
      try {
        const d = await fetchPointDetail({ keyword, id });
        if (d) store.setPointDetail(id, d);
      } catch (e) {
        log.warn(`Enrich fail id=${id}`, e.message);
      }
      setTimeout(tick, intervalMs);
    } else {
      // Rien à enrichir : on attend plus longtemps avant de refaire un tour
