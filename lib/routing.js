...
async function fetchForecastNear(lat, lon) {
  ...
  // Modèle = ECMWF IFS 0.25° (même famille que Windy par défaut). L'identifiant ecmwf_ifs04
  // est déprécié et retourne null partout ; ecmwf_ifs025 est le high-res prod actuel.
  const url = `${OM_FORECAST}?latitude=${latQ}&longitude=${lonQ}`
    + `&hourly=wind_speed_10m,wind_direction_10m&wind_speed_unit=kn`
    + `&models=ecmwf_ifs025&forecast_days=2&timezone=UTC`;
...
function windAt(hourly, timeMs) {
  ...
  // Direction : interp comme vecteur unitaire (composantes east=sin(dir), north=cos(dir)).
  // Convention compass : 0°=N (axe nord), 90°=E (axe est), donc bearing = atan2(east, north).
  const radB = Number(dBefore) * Math.PI / 180;
  const radA = Number(dAfter) * Math.PI / 180;
  const eastComp = Math.sin(radB) + frac * (Math.sin(radA) - Math.sin(radB));
  const northComp = Math.cos(radB) + frac * (Math.cos(radA) - Math.cos(radB));
  let direction = Math.atan2(eastComp, northComp) * 180 / Math.PI;
