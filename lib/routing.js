...
async function fetchForecastNear(lat, lon) {
  ...
  // Modèle = ECMWF IFS (même que Windy par défaut) pour cohérence visuelle.
  // GFS = défaut Open-Meteo (NOAA, 25 km), ECMWF IFS = ~10 km, souvent meilleur sur Europe.
  const url = `${OM_FORECAST}?latitude=${latQ}&longitude=${lonQ}`
    + `&hourly=wind_speed_10m,wind_direction_10m&wind_speed_unit=kn`
    + `&models=ecmwf_ifs04&forecast_days=2&timezone=UTC`;
